...
/**
 * Speed up an audio file using ffmpeg `atempo` filter (pitch-preserving).
 * `factor` range: 0.5 – 2.0 (values > 2.0 require chaining atempo filters).
 * Overwrites `inPath` in-place using a tmp file swap.
 */
export async function speedUpAudio(inPath: string, factor: number): Promise<void> {
  if (Math.abs(factor - 1.0) < 0.001) return; // no-op if factor ~= 1.0

  // ffmpeg atempo is limited to [0.5, 2.0] — chain if needed
