...
export interface ComposeArgs {
  ...
  gapSec: number;
  bgImageRelPath?: string | null;   // null or omitted => use CSS gradient
  audioRelPath: string;
...
export function composeHtml(args: ComposeArgs): string {
  ...
  const timing = script.scenes.map((scene, idx) => {
    ...
    const start = cursor;
    cursor += dur;
    const sceneIndexStr = String(idx + 1).padStart(2, "0");
    return { scene, sceneIndexStr, start, duration: dur };
  });
  const totalDuration = cursor;

  // Render scenes
  const sceneHtml = timing.map(({ scene, sceneIndexStr, start, duration }) => {
    return renderScene(scene, sceneIndexStr, start, duration, bgImageRelPath, tiktok, tiktokAvatar);
  }).join("\n");
...
function renderScene(
  ...
  switch (td.template) {
    case "hook":
      ...
      break;
    case "comparison":
      inner = renderBodyInner(renderComparisonInner(td));
      layoutName = "comparison";
      break;
    case "stat-hero":
      inner = renderBodyInner(renderStatHeroInner(td));
      layoutName = "stat-hero";
      break;
    case "feature-list":
      inner = renderBodyInner(renderFeatureListInner(td));
      layoutName = "feature-list";
      break;
    case "callout":
      inner = renderBodyInner(renderCalloutInner(td));
      layoutName = "callout";
      break;
    case "code-block":
      inner = renderBodyInner(renderCodeBlockInner(td));
      layoutName = "code-block";
      break;
    case "benchmark":
      inner = renderBodyInner(renderBenchmarkInner(td));
      layoutName = "benchmark";
      break;
    case "icon-grid":
      inner = renderBodyInner(renderIconGridInner(td));
      layoutName = "icon-grid";
      break;
    case "outro":
  ...
  return buildScene(scene, sceneIndexStr, start, duration, layoutName, inner);
}

// ── BODY SCENE WRAPPER ─────────────────────────────────────────────────────
function renderBodyInner(contentHtml: string): string {
  return contentHtml;
}
...
// ── CODE BLOCK SCENE ───────────────────────────────────────────────────────
function renderCodeBlockInner(td: Extract<TemplateDataType, { template: "code-block" }>): string {
  const filename = td.filename ?? "terminal";
  const linesHtml = td.lines.map((line, i) => {
    const trimmed = line.trim();
    const isComment = trimmed.startsWith("#") || trimmed.startsWith("//");
    const isCmd = trimmed.startsWith("claude") || trimmed.startsWith("npm") || trimmed.startsWith("npx") || trimmed.startsWith("git");
    const cls = isComment ? "code-comment" : isCmd ? "code-cmd" : "code-line";
    return `<div class="${cls}"><span class="code-ln">${i + 1}</span><span class="code-str">${escapeHtml(line)}</span></div>`;
  }).join("\n      ");

  return `
<div class="layout-code-block">
  <div class="code-card">
    <div class="code-header">
      ...
      <div class="code-title">${escapeHtml(filename)}</div>
    </div>
    <div class="code-title-main">${escapeHtml(td.title)}</div>
    <div class="code-body">
      ${linesHtml}
    </div>
  </div>
...
// ── BENCHMARK SCENE ────────────────────────────────────────────────────────
function renderBenchmarkInner(td: Extract<TemplateDataType, { template: "benchmark" }>): string {
  const pctNum = td.valueNumber ?? 64;
  const context = td.context ? `<div class="bench-context">${escapeHtml(td.context)}</div>` : "";
  return `
<div class="layout-benchmark">
  <div class="bench-card">
    <div class="bench-title">${escapeHtml(td.title)}</div>
    <div class="bench-meter-container">
      <div class="bench-track">
        <div class="bench-fill" style="width: ${pctNum}%"></div>
        <div class="bench-needle" style="left: ${pctNum}%"></div>
      </div>
    ...
    <div class="bench-value shimmer-sweep-target">${escapeHtml(td.percentage)}</div>
    <div class="bench-label">${escapeHtml(td.label)}</div>
    ${context}
  </div>
</div>`.trim();
}
...
// ── ICON GRID SCENE ────────────────────────────────────────────────────────
function renderIconGridInner(td: Extract<TemplateDataType, { template: "icon-grid" }>): string {
  const itemsHtml = td.items.map((item, i) => `
    <div class="grid-card grid-card-${i}" data-idx="${i}">
      <div class="grid-card-icon">${escapeHtml(item.icon)}</div>
      <div class="grid-card-label">${escapeHtml(item.label)}</div>
    </div>
  `).join("\n    ");

  return `
<div class="layout-icon-grid">
  <div class="grid-title">${escapeHtml(td.title)}</div>
